...
## 添加动画的属性
@export_flags("scale", "color", "offset","rotation") var anim_type = 1
...
@export var hover_rotation_degrees:float=0
var default_rotation_degrees:float=0
...
func reset_value():
	...
	default_rotation_degrees=target.rotation_degrees
...
func enter_hover():
	for i in select:
		if i == 1:
			...
			add_tween("rotation_degrees", hover_rotation_degrees, t_time)
...
func exit_hover():
	for i in select:
		if i == 1:
			...
			add_tween("rotation_degrees", default_rotation_degrees, t_time)
